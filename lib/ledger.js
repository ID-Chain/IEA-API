...
class PoolLedger {
    ...
    async proverGetEntitiesFromLedger(submitterDid, identifiers) {
        ...
        return this._getEntitiesFromLedger(submitterDid, identifiers, function(item) {
            return [0, now];
        });
    }
     ...
     * @param {String} submitterDid
     * @param {String} revocRegId
     * @param fromtime   starting time of accumulator's changes
     * @param totime  ending time of accumulator's changes
     * @return {Promise<void>}
     */
    async getRevocRegEntryDelta(submitterDid, revocRegId, fromtime, totime) {
        return this._get(
     ...
     * @param {String} submitterDid did to use for submitting requests to ledger
     * @param {Object[]} identifiers Array of objects containing schemaId, credDefId, and revRegId
     * @param {Function} timeWindowFn provides input for time window parameters of the algorithm
     * @return {Any[]} [schemas, credDefs,  revRegDefs, revRegs]
     */
    async getEntitiesFromLedger(submitterDid, identifiers, timeWindowFn) {
        let schemas = {};
        let credDefs = {};
        let revRegDefs = {};
        let revRegs = {};
        for (const item of identifiers) {
            ...
            if (item.rev_reg_id) {
                const [revRegId, revReg] = await this.getRevocRegDef(submitterDid, item['rev_reg_id']);
                revRegDefs[revRegId] = revReg;
                const [fromtime, totime] = timeWindowFn(item);
                const revRegDelta = await this.getRevocRegEntryDelta(
                    submitterDid,
                    item['rev_reg_id'],
                    fromtime,
                    totime
                );
                revRegs[revRegId][fromtime] = revRegDelta;
            }
        }
