...
class PoolLedger {
    ...
    async proverGetEntitiesFromLedger(submitterDid, identifiers) {
        ...
        return this._getEntitiesFromLedger(submitterDid, identifiers, item => [
            now,
            this.getRevocRegEntryDelta(submitterDid, item['rev_reg_id'], 0, now)
        ]);
    }
    ...
    async verifierGetEntitiesFromLedger(submitterDid, identifiers) {
        return this._getEntitiesFromLedger(submitterDid, identifiers, item => [
            item['timestamp'],
            this.getRevocRegEntry(submitterDid, item['rev_reg_id'], item['timestamp'])
        ]);
    }
     ...
     * @return {Promise<void>}
     */
    async getRevocRegEntry(submitterDid, revocRegId, totime) {
        return this._get(
    ...
    async getEntitiesFromLedger(submitterDid, identifiers, getAccumulatorFn) {
        ...
        for (const item of identifiers) {
            ...
            if (item.rev_reg_id) {
                ...
                const [fromtime, revRegDelta] = getAccumulatorFn(item);
