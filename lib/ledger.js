    ...
    async verifierGetEntitiesFromLedger(submitterDid, identifiers) {
        ...
        for (const item of identifiers) {
            ...
            if (item.rev_reg_id) {
                ...
                const [, revReg, timestamp] = await this.getRevocReg(submitterDid, item.rev_reg_id, now);
                log.debug(`Timestamp for verifier get entitites get Revoc Registry: ${timestamp}, param (now): ${now}`);
