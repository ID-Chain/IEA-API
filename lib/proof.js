 ...
 * @param {number} from beginning of interval to pull revocation states from ledger
 * @param {number} to end of interval to pull revocation states from ledger
 * @return {Promise<Any[]>} [selfAttestededAttributes, requestedFields]
 */
async function buildRequestedFields(submitterDid, ledger, searchHandle, requestFields, values, from, to) {
    const schemas = {};
    ...
    for (const referent of Object.keys(requestFields)) {
        ...
        if (result.length > 0) {
            ...
            if (credential.rev_reg_id) {
                ...
                const fileExists = await new Promise(resolve => fs.access(filePath, err => resolve(!err)));
                if (!fileExists) {
...
module.exports = {
    ...
    async createProof(walletHandle, ledger, masterSecretId, recipientDid, proofRequest, values = {}) {
        ...
        let from;
        let to;
        if (proofRequest.non_revoked !== undefined) {
            from = proofRequest.non_revoked.from === undefined ? 0 : proofRequest.non_revoked.from;
            to =
                proofRequest.non_revoked.to === undefined ? Math.floor(Date.now() / 1000) : proofRequest.non_revoked.to;
        } else {
            from = 0;
        ...
        // Object.keys(requestedAttributes).map(k => (requestedAttributes[k].timestamp = now));
        // Object.keys(requestedPredicates).map(k => (requestedPredicates[k].timestamp = now));
        const requestedCredentials = {
            self_attested_attributes: Object.assign({}, selfAttestedAttributes, selfAttestedPredicates),
            requested_attributes: requestedAttributes,
            requested_predicates: requestedPredicates
        };
        log.debug('requestedCredentials:', requestedCredentials);
