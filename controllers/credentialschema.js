...
module.exports = {
  ...
  createAndStoreSchemaToLedger: wrap(async (req) => {
    ...
    let [schemaId, schema] = await indy.issuerCreateSchema(
      req.wallet.ownDid, credSchema.name, credSchema.version, credSchema.attrNames);
    const request = await indy.buildSchemaRequest(req.wallet.ownDid, schema);
    let response = await indy.signAndSubmitRequest(
      pool.handle, req.wallet.handle, req.wallet.ownDid, request);
