...
module.exports = {
    ...
    issue: wrap(async (req, res, next) => {
        ...
        const isRevocable =
            typeof credDefDoc.revocRegId !== 'undefined' && typeof credDefDoc.revocRegType !== 'undefined';
        ...
        if (isRevocable) {
            await pool.revocRegEntryRequest(
                req.wallet.handle,
                req.wallet.ownDid,
                credDefDoc.revocRegId,
                credDefDoc.revocRegType,
                revocRegDelta
            );
        }
    ...
    store: wrap(async (req, res, next) => {
        ...
        const [holderDid, holderVk, issuerVerKey, credential] = await req.wallet.tryAuthDecrypt(
            req.body.encryptedCredential
        );
