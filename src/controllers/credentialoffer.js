...
module.exports = {
    ...
    accept: wrap(async (req, res, next) => {
        const [
            ...
            credDefId,
            credOfferId
        ] = await module.exports.acceptCredentialOfferAndCreateCredentialRequest(req);
        let credReq = new CredReq({
            wallet: req.wallet.id,
            credDefId: credDefId,
            credOfferId: credOfferId,
            credReqMetaData: credReqMeta,
            data: cryptedCredReq.toString('base64')
        });
    ...
    acceptCredentialOfferAndCreateCredentialRequest: wrap(async (req, res, next) => {
        const [holderDid, holderKey, issuerVerKey, credOfferJson] = await req.wallet.tryAuthDecrypt(
            req.body.encryptedCredentialOffer
        );
        ...
        const [credReq, credReqMeta] = await indy.proverCreateCredentialReq(
            req.wallet.handle,
            holderDid,
            credOfferJson,
            credDef,
        ...
        const authCryptedCredRequest = await indy.cryptoAuthCrypt(
            req.wallet.handle,
            holderKey,
            issuerVerKey,
            bufferCredReq
    ...
    getCredentialOffer: wrap(async req => {
        ...
        const [senderVK, decryptedCredOffer] = await indy.cryptoAuthDecrypt(
            req.wallet.handle,
            recipientVK,
            credOffer['data']
        );
